"""Prometheus metric definitions for the GenAI service (issue #54).
...
2. GenAI-specific — defined here:
   ...
   - `genai_provider_requests_total{provider, endpoint, outcome}` —
     every call to the LLM provider, labelled by outcome
     (ok / timeout / rate_limit / unavailable / bad_request / error).
   - `genai_provider_request_duration_seconds{provider, endpoint}` —
     latency of the provider call itself, separate from end-to-end
     HTTP request latency.
   - `genai_validation_errors_total{endpoint, reason}` —
     structured-output validation failures, by reason
     (json_decode / wrong_type / schema).
...
provider_requests_total = Counter(
    ...
    "GenAI provider invocations, by endpoint, provider, and outcome.",
...
validation_errors_total = Counter(
    ...
    "Model-output validation failures, by endpoint and reason.",
...
async def observe_provider_call(
    ...
    *outside* this context so they do not skew provider-side accounting.
    ...
    try:
        ...
        provider_request_duration_seconds.labels(provider, endpoint).observe(elapsed)
        provider_requests_total.labels(provider, endpoint, outcome).inc()
