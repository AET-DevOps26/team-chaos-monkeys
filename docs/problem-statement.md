...
**c) Outbound message generation.** When a match is confirmed, the notification service calls the GenAI component to generate the pickup email or SMS in the guest's language and in a tone appropriate to the venue (a transit hub is terse; a hotel is formal). Static templates handle the common path; the LLM earns its keep on the long tail of languages and tone variation between venues.
