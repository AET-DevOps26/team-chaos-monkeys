...
.
├── api/
│   └── openapi.yaml            — GenAI service Contract (spring paths land in #61)
...
`api/openapi.yaml` is the single source of truth for synchronous APIs. The Project runs a mixed project model today (see #61 for planned reconciliation).
...
- **Releases:** merging `development → main` triggers continuous deployment to Kubernetes. Reserve `main` for release PRs. (Planned for future)
